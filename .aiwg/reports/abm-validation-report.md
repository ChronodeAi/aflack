...
| Architecture baseline | `.aiwg/architecture/software-architecture-document.md` plus ADR-0001 through ADR-0007 | PASS |
...
|---|---|---|
| Architecture baselined and stable | PASS | SAD and seven ADRs establish scheduler, event store, memory substrate, director runtime, human-gated orchestration, virality doctrine, and memory bakeoff posture. |
...
| Test approach established | PASS | Test strategy exists and the local unit suite now covers Postiz URL normalization/payload preview, learning hash/credibility logic, and Aside scan pure logic. Compliance smoke remains the deterministic gate for prohibited samples. |
...
4. Continue adding focused automated tests before expanding the publisher/generation surface.
