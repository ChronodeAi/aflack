...
tools: ["Create","Edit","Execute","Glob","Grep","Read"]
---
...
### Auth.log Brute Force Detection (anchor)

```bash
grep "Failed password" auth.log | awk '{print $11}' | sort | uniq -c | sort -rn | head -10
# 4821 185.220.101.47   1205 91.108.4.12   843 194.165.16.11
grep "185.220.101.47" auth.log | grep -E "Accepted|opened"
# Accepted password for admin from 185.220.101.47 port 51284 ssh2  (Mar 15 03:47:22)
```

**Finding**: 185.220.101.47 ran 4,821 attempts and succeeded at 03:47:22 UTC. ATT&CK: T1110.001 (Brute Force) + T1078 (Valid Accounts). Escalate IP as confirmed attacker IOC.

> Additional worked examples (brute force + multi-source correlation): see `docs/agent-examples/log-analyst-examples.md` (`aiwg discover "log analyst worked examples"`).
